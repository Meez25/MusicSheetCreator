...
const Cell: React.FC<{ borderBottom?: boolean, selectColor: string }> = ({ borderBottom, selectColor }) => {
  ...
  return (
    <div
      ...
      className={`h-6 w-6 border box-border flex items-center justify-center ${isABorderColor(assignedColor) && clicked ? "border-4" : ""} ${clicked ? `${assignedColor} rounded-full` : ""}`}
    >
      {borderBottom && <div className="box-content bg-black min-h-[2px] w-full"></div>}
    </div>
  );
};
