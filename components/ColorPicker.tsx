...
export const ColorPicker: React.FC<ColorPickerProps> = ({ setSelectedColor }) => {
    ...
    <>
      ...
      <div className="flex flex-row justify-center my-5 gap-1">
        {bgColors.map((color, i) => {
          return (
            <ColoredButtons key={i} fill color={color} selectColor={setSelectedColor} />
          )
        })}
      </div>
      ...
      <div className="flex flex-row justify-center gap-1">
        {borderColors.map((color, i) => (
          <ColoredButtons key={i} color={color} fill={false} selectColor={setSelectedColor} />
        ))}
      </div>
    </>
  )
